...
import { FiArrowLeft, FiUpload, FiPackage, FiTag, FiDollarSign, FiPlus, FiTrash2, FiUser, FiInfo, FiLayers, FiAlertCircle, FiShield } from 'react-icons/fi';
...
export default function AdminNewProduct() {
    ...
    const loadData = async () => {
      try {
        const [catsRes, sellersRes] = await Promise.all([
          ...
          adminAPI.getSellers({ status: 'approved', limit: 100 })
        ]);
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    if (!form.seller_id) return toast.error('Please assign a seller');
      ...
      <div className="bg-white border-b border-dark-100 py-8 mb-8">
        <div className="max-w-5xl mx-auto px-4 flex items-center justify-between">
          <div className="flex items-center gap-4">
            <Link href="/admin/products" className="w-10 h-10 rounded-full border border-dark-100 flex items-center justify-center hover:bg-dark-50 transition-all"><FiArrowLeft /></Link>
            <div>
              <h1 className="text-2xl font-black text-dark-900 tracking-tight">Admin: Add New Product</h1>
              <p className="text-xs text-dark-500 font-bold uppercase tracking-widest mt-0.5">Super Admin Dashboard</p>
            </div>
          </div>
        ...
        <form onSubmit={handleSubmit} className="grid lg:grid-cols-12 gap-8 items-start">
          ...
          <div className="lg:col-span-8 space-y-6">
            ...
            <div className="bg-white rounded-[32px] shadow-sm border border-dark-100 overflow-hidden">
               <div className="p-6 border-b border-dark-50 bg-dark-50/30 flex items-center gap-3">
                  <FiUser className="text-[#fb641b]" />
                  <h2 className="text-sm font-black text-dark-800 uppercase tracking-tight">Assign Seller & Basic Info</h2>
               </div>
               <div className="p-8 space-y-6">
                  <div className="p-4 bg-orange-50 border border-orange-100 rounded-2xl flex items-start gap-3">
                     <FiAlertCircle className="text-[#fb641b] mt-1" />
                     <p className="text-xs font-bold text-orange-800 leading-relaxed">As an Admin, you are listing this product on behalf of a seller. Please ensure the correct store is selected.</p>
                  </div>

                  <div>
                    <label className={labelClass}>Select Seller *</label>
                    <select value={form.seller_id} onChange={(e) => update('seller_id', e.target.value)} className={inputClass} required>
                      <option value="">Choose a store to assign this product</option>
                      {sellers.map((s) => (
                        <option key={s.id} value={s.id}>
                          {s.store_name} ({s.user?.name})
                        </option>
                      ))}
                    </select>
                  </div>

                  <div className="w-full h-[1px] bg-dark-50 my-6"></div>

                  <div>
                    <label className={labelClass}>Product Title *</label>
                    <input value={form.name} onChange={(e) => update('name', e.target.value)} className={inputClass} placeholder="e.g. Fresh Red Apples" required />
                  </div>
                  <div>
                    <label className={labelClass}>Description</label>
                    <textarea value={form.description} onChange={(e) => update('description', e.target.value)} className={inputClass} rows={5} placeholder="Describe the product features, quality, etc." />
                  </div>
               </div>
            </div>
            ...
            <div className="bg-white rounded-[32px] shadow-sm border border-dark-100 overflow-hidden">
               <div className="p-6 border-b border-dark-50 bg-dark-50/30 flex items-center gap-3">
                  ...
                  <h2 className="text-sm font-black text-dark-800 uppercase tracking-tight">Pricing & Inventory</h2>
               </div>
               <div className="p-8">
                  <div className="grid sm:grid-cols-2 gap-6 mb-8">
                    ...
                    <div>
                      <label className={labelClass}>MRP / Compare Price (₹)</label>
                  ...
                  <div className="grid grid-cols-2 sm:grid-cols-3 gap-6">
                    ...
                    <div className="col-span-2 sm:col-span-1">
                      ...
                      <input value={form.sku} onChange={(e) => update('sku', e.target.value)} className={inputClass} placeholder="SKU-123" />
                    </div>
                  </div>
               </div>
            </div>
                  ...
                  <div className="grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-5 gap-5">
                    {previews.map((src, i) => (
                      <div key={i} className="relative aspect-square rounded-[24px] overflow-hidden border-2 border-dark-50 group shadow-sm">
                        ...
                        {i === 0 && <span className="absolute bottom-2 left-2 bg-[#fb641b] text-white text-[8px] font-black uppercase px-2 py-1 rounded-md shadow-lg">Main</span>}
                      </div>
                    ))}
                    {files.length < 5 && (
                      <label className="aspect-square rounded-[24px] border-2 border-dashed border-dark-100 flex flex-col items-center justify-center gap-2 cursor-pointer hover:bg-dark-50 hover:border-[#fb641b] transition-all group">
                        ...
                        <span className="text-[10px] font-black text-dark-400 uppercase tracking-widest">Upload</span>
                        <input type="file" multiple accept="image/*" onChange={handleFileChange} className="hidden" />
                      </label>
                    )}
                  </div>
                  <p className="mt-8 text-[11px] text-dark-400 font-bold uppercase tracking-[0.1em] flex items-center gap-2"><FiInfo className="text-[#fb641b]" /> Recommended size: 1000x1000px. Max 5 images.</p>
               </div>
            </div>
          </div>
          ...
          <div className="lg:col-span-4 space-y-6 lg:sticky lg:top-24">
             <div className="bg-white rounded-[32px] shadow-sm border border-dark-100 overflow-hidden">
                ...
                <div className="p-6 space-y-6">
                   ...
                   <div>
                      ...
                      {!form.category_id && <p className="text-[10px] text-orange-500 font-bold mt-2 uppercase tracking-wider">Select main category first</p>}
                   </div>
                   
                   <div className="pt-4 border-t border-dark-50">
                      <label className="flex items-center gap-3 cursor-pointer group">
                         <div className={`w-12 h-6 rounded-full transition-all relative ${form.is_active ? 'bg-[#fb641b]' : 'bg-dark-200'}`}>
                         ...
                         <span className="text-sm font-black text-dark-700 uppercase tracking-widest">Active Product</span>
                      </label>
                      <p className="text-[10px] text-dark-400 mt-2 font-medium">Inactive products will not be visible to customers.</p>
                   </div>
                </div>
             </div>
             ...
             <div className="bg-white rounded-[32px] p-8 shadow-sm border border-dark-100 space-y-4">
                ...
                <button type="button" onClick={() => router.push('/admin/products')} className="w-full py-4 rounded-2xl font-black text-sm text-dark-400 hover:text-dark-900 transition-all uppercase tracking-widest">
                   Discard Draft
                </button>
             </div>
          </div>
